...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate category
    const validCategories: CategoryId[] = ['risk-analysis', 'clause-extraction', 'compliance-review', 'contract-comparison', 'summary-insights', 'negotiation-support'];
    if (!validCategories.includes(category)) {
      return NextResponse.json(
        { error: 'Invalid category. Must be one of: risk-analysis, clause-extraction, compliance-review, contract-comparison, summary-insights, negotiation-support' },
        { status: 400 }
      );
    }
