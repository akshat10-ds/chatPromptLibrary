...
import { Sparkles, Gauge, Bot, Clock, Zap } from 'lucide-react';

const toolLabels: Record<ToolId, string> = {
  'docusign-ai': 'Docusign AI',
  claude: 'Claude',
  chatgpt: 'ChatGPT',
  gemini: 'Gemini',
  other: 'Other',
  ...
  return (
    <Link href={`/prompt/${prompt.id}`} className="block">
      <article className="prompt-card p-5 flex flex-col h-full group">
        ...
        <h3 className="font-serif text-lg text-text-primary mb-1 group-hover:text-text-secondary transition-colors">
          {prompt.title}
        </h3>
        ...
        {/* Value Proposition */}
        {prompt.valueProp && (
          <div className="value-prop mb-3">
            <Zap size={14} />
            <span>{prompt.valueProp}</span>
          </div>
        )}
        ...
        {/* Metadata Row */}
        {(prompt.tools || prompt.difficulty || prompt.timeToComplete) && (
